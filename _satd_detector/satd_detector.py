...
class SATDDetector:
    ...
    def classify_prob_comment(self, comment, tp=None):
        ...
        embed_text_np = np.array(embed_text)
        x = torch.tensor(embed_text_np, requires_grad=True)
        x = x.unsqueeze(0)
        if self.params.cuda:
            x = x.cuda()
        output = self.model(x)
...
def get_params():
    ...
    parser.add_argument('--embed-vectors', type=str, default="",
...
# loads a csv file from path and runs the detector on it
def runFile(filePath, outputPath):
    d = SATDDetector(get_params())
    with open(filePath, 'r') as file:
        # Create a CSV reader object
        csv_reader = csv.reader(file)

        # Iterate over rows in the CSV file and run detector on each row
        for row in csv_reader:
            if(len(row) >3):

                predicted = d.classify_prob_comment(row[3], tp=DEF_COMMENT)
                if(predicted.item()): # 0 is non-SATD
            
                    # print('Source type: {}\nText: {}'.format(tp, comment))
                    # print('Predicted result: {}\n'.format(DEF_LABELS[predicted.item()]))
                    # print('------------------------')
                    data_to_add = [[row[0],row[1],row[2], row[3],DEF_COMMENT, DEF_LABELS[predicted.item()]]]
                    with open(outputPath, 'a', newline='') as file:
                        # Create a CSV writer object
                        csv_writer = csv.writer(file)

                        # Write the data to the CSV file
                        csv_writer.writerows(data_to_add)
